...
app = FastAPI(title="SolvX Ocean Data API", description="API for interactive 3D ocean visualization", version="2.2.4")
...
def normalize_time_coordinate(data):
    ...
    try:
        ...
        values=pd.DatetimeIndex(raw).tz_localize(None)
...
def normalize_time_value(value):
    ...
    try:
        if isinstance(value,(int,float,np.integer,np.floating)):
            n=float(value); a=abs(n); unit="ns" if a>=1e17 else "us" if a>=1e14 else "ms" if a>=1e11 else "s"; parsed=pd.to_datetime(n,unit=unit,utc=True)
        else:
            text=str(value).strip()
            if text.isdigit() and len(text)>=10:
                n=float(text); a=abs(n); unit="ns" if a>=1e17 else "us" if a>=1e14 else "ms" if a>=1e11 else "s"; parsed=pd.to_datetime(n,unit=unit,utc=True)
            else: parsed=pd.to_datetime(text,utc=True)
        if isinstance(parsed,pd.DatetimeIndex):return parsed.tz_localize(None)
        if getattr(parsed,"tzinfo",None) is not None:parsed=parsed.tz_convert(None)
        return parsed
...
def select_time(data,value,method="nearest"):
    ...
    data=normalize_time_coordinate(data); return data.sel(time=normalize_time_value(value),method=method)
...
def find_current_components():
    ...
    f=matches[0][0];names=[m[1] for m in matches if m[0]==f]
...
def metadata(filename:str):
    with open_dataset(filename) as ds:
        ...
        for name,c in ds.coords.items():
            vals=c.values;step=max(1,vals.size//5000);coords[name]={"size":int(vals.size),"min":sanitize(vals.min()) if vals.size else None,"max":sanitize(vals.max()) if vals.size else None,"values":sanitize(vals.tolist() if name=="time" or vals.size<=5000 else vals[::step].tolist()),"units":c.attrs.get("units")}
...
def ocean_catalog():
    ...
    for logical,label in labels:
        ...
        if not matches:out.append({"id":logical,"label":label,"available":False,"reason":"No matching NetCDF variable found"});continue
        f,n,attrs,dims,shape=matches[0];out.append({"id":logical,"label":label,"available":True,"file":f.name,"variable":n,"units":attrs.get("units"),"long_name":attrs.get("long_name"),"standard_name":attrs.get("standard_name"),"dimensions":dims,"shape":shape,"matches":[{"file":x[0].name,"variable":x[1],"units":x[2].get("units"),"dimensions":x[3],"shape":x[4]} for x in matches]})
    ...
    for f,n,*_ in find_logical("temperature"):
        with xr.open_dataset(f) as ds:
            if "time" in ds[n].dims and "time" in ds.coords:
                vals=normalize_time_coordinate(ds[n]).coords["time"].values;return {"file":f.name,"variable":n,"count":int(vals.size),"values":[pd.Timestamp(v).isoformat() for v in vals]}
    for f in get_nc_files():
        try:
            with xr.open_dataset(f) as ds:
                if "time" in ds.coords:
                    vals=normalize_time_coordinate(xr.DataArray(np.zeros(ds.time.size),dims=["time"],coords={"time":ds.time.values})).coords["time"].values;return {"file":f.name,"variable":None,"count":int(vals.size),"values":[pd.Timestamp(v).isoformat() for v in vals]}
        except Exception:pass
...
def ocean_current_grid(time:Optional[str]=None,depth:Optional[float]=None,stride:int=3):
    ...
    with xr.open_dataset(f) as ds:
        ...
        ydim="latitude" if "latitude" in u.dims else "lat" if "lat" in u.dims else None;xdim="longitude" if "longitude" in u.dims else "lon" if "lon" in u.dims else None
        if not ydim or not xdim:raise HTTPException(422,detail="Current dataset has no latitude/longitude dimensions")
        u=u.isel({ydim:slice(None,None,stride),xdim:slice(None,None,stride)}).squeeze();v=v.isel({ydim:slice(None,None,stride),xdim:slice(None,None,stride)}).squeeze();lat,lon=u.coords.get(ydim),u.coords.get(xdim)
...
def get_region_array(file:str,variable:str,lat_min:Optional[float]=None,lat_max:Optional[float]=None,lon_min:Optional[float]=None,lon_max:Optional[float]=None,depth_min:Optional[float]=None,depth_max:Optional[float]=None,time_start:Optional[str]=None,time_end:Optional[str]=None,stride:int=1):
    with open_dataset(file) as ds:
        ...
        values=np.asarray(data.values,dtype=np.float32);return {"file":file,"variable":variable,"dimensions":list(data.dims),"shape":list(values.shape),"dtype":str(values.dtype),"coordinates":{d:sanitize(data[d].values) for d in data.dims if d in data.coords},"data":sanitize(values)}
