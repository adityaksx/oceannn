...
function project(lon,lat){const b=S.g.bounds,ml=(b[2]+b[3])/2,klon=111.32*Math.cos(ml*Math.PI/180);return[(n(lon)-(b[0]+b[1])/2)*klon,(n(lat)-(b[2]+b[3])/2)*111.32]}
...
function drawLand(){dispose(S.land);dispose(S.landSides);dispose(S.landBottom);const parts=[...(S.g.land||[]),...(S.g.islands||[])],top=.35,thickness=Math.max(2,n(S.g.landThickness||3)*S.depthEx/1000),bottom=top-thickness;
 S.land=new THREE.Mesh(polyGeo(parts,top),new THREE.MeshBasicMaterial({color:0x32b449,side:THREE.DoubleSide}));S.land.renderOrder=60;S.root.add(S.land);
 const p=[],ix=[];for(const part of parts){const r=part.top||[];for(let i=0;i<r.length-1;i++){const a=r[i],b=r[i+1],q=p.length/3;p.push(n(a[0]),top,n(a[1]),n(a[0]),bottom,n(a[1]),n(b[0]),top,n(b[1]),n(b[0]),bottom,n(b[1]));ix.push(q,q+2,q+1,q+2,q+3,q+1)}}
 const sg=new THREE.BufferGeometry();sg.setAttribute('position',new THREE.Float32BufferAttribute(p,3));sg.setIndex(ix);S.landSides=new THREE.Mesh(sg,new THREE.MeshBasicMaterial({color:0x6b4528,side:THREE.DoubleSide}));S.landSides.renderOrder=55;S.root.add(S.landSides);
 S.landBottom=new THREE.Mesh(polyGeo(parts,bottom),new THREE.MeshBasicMaterial({color:0x56371f,side:THREE.DoubleSide}));S.landBottom.renderOrder=54;S.root.add(S.landBottom)
}
function drawCoast(){dispose(S.coast);const lines=[...(S.g.coast||[]),...(S.g.landBoundary||[]),...(S.g.islandCoast||[])];S.coast=new THREE.LineSegments(lineGeo(lines,.48),new THREE.LineBasicMaterial({color:0x12351e}));S.coast.renderOrder=90;S.root.add(S.coast)}
function drawSeabed(){dispose(S.seabed);const t=S.g.terrain,nx=t.x.length,ny=t.y.length,p=[],ix=[],map=new Int32Array(nx*ny);map.fill(-1);for(let j=0;j<ny;j++)for(let i=0;i<nx;i++){const d=n((t.rawDepthKm[j]||[])[i]);if(!finite(d))continue;map[j*nx+i]=p.length/3;p.push(t.x[i],depthZ(d*1000),t.y[j])}for(let j=0;j<ny-1;j++)for(let i=0;i<nx-1;i++){const a=map[j*nx+i],b=map[j*nx+i+1],c=map[(j+1)*nx+i],d=map[(j+1)*nx+i+1];if(a>=0&&b>=0&&c>=0&&d>=0)ix.push(a,c,b,b,c,d)}const g=new THREE.BufferGeometry();g.setAttribute('position',new THREE.Float32BufferAttribute(p,3));g.setIndex(ix);g.computeVertexNormals();S.seabed=new THREE.Mesh(g,new THREE.MeshStandardMaterial({color:0x85572f,roughness:1,side:THREE.DoubleSide}));S.seabed.renderOrder=8;S.root.add(S.seabed)}

function buildWater(){dispose(S.water);S.water=new THREE.Group();const t=S.g.terrain,nx=t.x.length,ny=t.y.length,raw=t.rawDepthKm,step=Math.max(1,Math.ceil(Math.sqrt(nx*ny/18000))),cells=[];
 for(let j=0;j<ny-1;j+=step)for(let i=0;i<nx-1;i+=step){const i2=Math.min(nx-1,i+step),j2=Math.min(ny-1,j+step),ds=[];for(let y=j;y<=j2;y++)for(let x=i;x<=i2;x++){const d=n((raw[y]||[])[x]);if(finite(d)&&d>0)ds.push(d)}if(!ds.length)continue;const depthKm=Math.min(...ds);if(depthKm<.005)continue;const x0=t.x[i],x1=t.x[i2],y0=t.y[j],y1=t.y[j2],h=Math.abs(depthZ(depthKm*1000));if(h<.15)continue;cells.push({x:(x0+x1)/2,y:(y0+y1)/2,sx:Math.max(.3,Math.abs(x1-x0)*.985),sy:Math.max(.3,Math.abs(y1-y0)*.985),h,depthKm})}
 if(!cells.length)throw Error('No ocean cells were found in GEBCO geometry');
 const box=new THREE.BoxGeometry(1,1,1),mat=new THREE.MeshPhysicalMaterial({color:0x208fb5,transparent:true,opacity:.34,roughness:.28,metalness:0,transmission:.04,ior:1.333,depthWrite:false,side:THREE.DoubleSide}),mesh=new THREE.InstancedMesh(box,mat,cells.length),dummy=new THREE.Object3D();
 for(let k=0;k<cells.length;k++){const c=cells[k];dummy.position.set(c.x,-c.h/2,c.y);dummy.scale.set(c.sx,c.h,c.sy);dummy.updateMatrix();mesh.setMatrixAt(k,dummy.matrix)}mesh.instanceMatrix.needsUpdate=true;mesh.userData={isWater:true,cells};mesh.renderOrder=20;S.water.add(mesh);S.root.add(S.water);box.dispose()
}
...
function drawChunkBase(){
 ...
 const baseZ=depthZ((maxDepth+1.0)*1000);
 const sideDepth=Math.abs(baseZ);
 ...
 S.chunkSides=new THREE.Mesh(g,new THREE.MeshBasicMaterial({color:0x664426,side:THREE.DoubleSide}));S.chunkSides.renderOrder=1;S.root.add(S.chunkSides);
 const bg=new THREE.BufferGeometry();bg.setAttribute('position',new THREE.Float32BufferAttribute([x0,baseZ,y0,x1,baseZ,y0,x1,baseZ,y1,x0,baseZ,y1],3));bg.setIndex([0,2,1,0,3,2]);S.chunkBottom=new THREE.Mesh(bg,new THREE.MeshBasicMaterial({color:0x593a24,side:THREE.DoubleSide}));S.chunkBottom.renderOrder=0;S.root.add(S.chunkBottom);
}

function renderVars(){const host=$('vars');if(!host)return;host.innerHTML='';const icons={temperature:'T',temperature_anomaly:'∆',salinity:'S',currents:'C',sea_level:'η',chlorophyll:'Ch'};for(const x of S.catalog){const b=document.createElement('button');b.className=`var ${x.id===S.active?'active':''} ${x.available===false?'off':''}`;b.disabled=x.available===false;b.innerHTML=`<span class="vicon">${icons[x.id]||'•'}</span><span><b>${x.label||x.id}</b><small>${x.units||'inspection only'}</small></span><span class="dot"></span>`;b.onclick=()=>{S.active=x.id;renderVars();status(`${x.label||x.id} · inspection only`)};host.appendChild(b)}}
...
function fit(){const box=new THREE.Box3().setFromObject(S.root);if(box.isEmpty())return;const c=box.getCenter(new THREE.Vector3()),s=box.getSize(new THREE.Vector3()),r=Math.max(s.x,s.y,s.z,1);S.controls.target.copy(c);S.camera.position.set(c.x+r*1.15,c.y-r*.72,c.z+r*.95);S.camera.lookAt(c);S.controls.update()}
function views(){document.querySelectorAll('#views .view').forEach(b=>b.onclick=()=>{const box=new THREE.Box3().setFromObject(S.root),c=box.getCenter(new THREE.Vector3()),s=box.getSize(new THREE.Vector3()),r=Math.max(s.x,s.y,s.z,1),v=b.dataset.view;if(v==='top')S.camera.position.set(c.x,c.y,c.z+r*1.45);else if(v==='profile')S.camera.position.set(c.x+r*1.5,c.y,c.z+.05*r);else if(v==='under')S.camera.position.set(c.x+r*.75,c.y-r*.55,c.z-r*1.2);else S.camera.position.set(c.x+r*1.15,c.y-r*.72,c.z+r*.95);S.controls.target.copy(c);S.controls.update();document.querySelectorAll('#views .view').forEach(x=>x.classList.remove('active'));b.classList.add('active')})}
...
async function init(){
 ...
 drawChunkBase();drawSeabed();drawLand();drawCoast();buildWater();
