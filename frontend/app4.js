...
// Coordinate system used by geometry.json: X=longitude/east-west, Y=vertical/depth, Z=latitude/north-south.
...
function drawLand(){
  ...
  const green=new THREE.MeshBasicMaterial({color:0x39b54a,side:THREE.DoubleSide});
  const soil=new THREE.MeshBasicMaterial({color:0xc39a6b,side:THREE.DoubleSide});

  S.land=new THREE.Mesh(polyGeo(parts,top),green);
  S.land.renderOrder=50;S.root.add(S.land);
  ...
  S.landSides=new THREE.Mesh(sg,soil);S.landSides.renderOrder=45;S.root.add(S.landSides);
  S.landBottom=new THREE.Mesh(polyGeo(parts,bottom),soil);S.landBottom.renderOrder=44;S.root.add(S.landBottom);
}

function drawCoast(){
  dispose(S.coast);
  const lines=[...(S.g.coast||[]),...(S.g.landBoundary||[]),...(S.g.islandCoast||[])];
  S.coast=new THREE.LineSegments(lineGeo(lines,.02),new THREE.LineBasicMaterial({color:0x18351d}));
...
function drawSeabed(){
  ...
  for(let j=0;j<ny;j++)for(let i=0;i<nx;i++){
    ...
    map[j*nx+i]=p.length/3;
    p.push(t.x[i],depthY(d*1000),t.y[j]);
  }
  ...
  S.seabed=new THREE.Mesh(g,new THREE.MeshBasicMaterial({color:0xc8a477,side:THREE.DoubleSide}));
  S.seabed.renderOrder=8;S.root.add(S.seabed);
}

// Translucent blue water. No transmission/refraction is used here: WebGL transmission without
// an environment can render as black. The water is built as separate prisms clipped to ocean cells.
function buildWater(){
  dispose(S.water);S.water=new THREE.Group();
  const t=S.g.terrain,nx=t.x.length,ny=t.y.length,raw=t.rawDepthKm;
  const step=Math.max(1,Math.ceil(Math.sqrt(nx*ny/14000))),cells=[];
  for(let j=0;j<ny-1;j+=step)for(let i=0;i<nx-1;i+=step){
    ...
    const depthKm=Math.min(...corners),h=Math.abs(depthY(depthKm*1000));
    if(h<.08)continue;
    cells.push({x:(t.x[i]+t.x[i2])/2,z:(t.y[j]+t.y[j2])/2,sx:Math.max(.25,Math.abs(t.x[i2]-t.x[i])*.995),sz:Math.max(.25,Math.abs(t.y[j2]-t.y[j])*.995),h,depthKm});
  }
  if(!cells.length)throw Error('No ocean cells were found in GEBCO geometry');

  const box=new THREE.BoxGeometry(1,1,1);
  // Basic material avoids the black/transmission artifact while remaining visibly translucent.
  const mat=new THREE.MeshBasicMaterial({color:0x2f9fd0,transparent:true,opacity:.24,depthWrite:false,side:THREE.DoubleSide});
  ...
  mesh.renderOrder=20;
  S.water.add(mesh);S.root.add(S.water);
}

// No rectangle is painted across the whole ocean at sea level. This was the source of the
// giant artificial plane/closure. The real seabed remains the visible geological bottom.
function drawChunkBase(){
  dispose(S.chunkSides);dispose(S.chunkBottom);
  S.chunkSides=null;S.chunkBottom=null;
}
...
function renderVars(){
  ...
  for(const x of S.catalog){
    ...
    b.onclick=()=>{S.active=x.id;renderVars();status(`${x.label||x.id} · inspection only`)};
    host.appendChild(b);
  }
}
...
async function inspect(lon,lat){
  try{
    ...
    const q=new URLSearchParams({longitude:String(lon),latitude:String(lat)});
    if(S.times[S.ti])q.set('time',S.times[S.ti]);
    showPoint(await json(`${API}/ocean/point?${q}`),lon,lat);
...
function clickOcean(e){
  ...
  S.mouse.x=(e.clientX-r.left)/r.width*2-1;S.mouse.y=-(e.clientY-r.top)/r.height*2+1;
  S.ray.setFromCamera(S.mouse,S.camera);
  const hit=S.ray.intersectObjects(S.water.children,true).find(h=>h.object?.userData?.isWater&&h.instanceId!=null);
  if(!hit)return;
  const c=hit.object.userData.cells[hit.instanceId],b=S.g.bounds,ml=(b[2]+b[3])/2,klon=111.32*Math.cos(ml*Math.PI/180);
  inspect((b[0]+b[1])/2+c.x/klon,(b[2]+b[3])/2+c.z/111.32);
}

function fit(){
  const box=new THREE.Box3().setFromObject(S.root);if(box.isEmpty())return;
  const c=box.getCenter(new THREE.Vector3()),s=box.getSize(new THREE.Vector3());
  const horizontal=Math.max(s.x,s.z,1),vertical=Math.max(s.y,1),r=Math.max(horizontal,vertical);
  S.controls.target.set(c.x,c.y*.12,c.z);
  // Flip the default 3D orientation from the previous build: ocean faces away and land is readable.
  S.camera.position.set(c.x+r*.95,c.y-r*.62,c.z-r*1.05);
...
function ui(){
  ...
  $('exaggeration')?.addEventListener('input',e=>{
    S.depthEx=n(e.target.value);if($('exagValue'))$('exagValue').textContent=`${S.depthEx}×`;
    drawLand();drawSeabed();buildWater();drawChunkBase();drawCoast();fit();
  });
  $('play')?.addEventListener('click',()=>{S.playing=!S.playing;$('play').textContent=S.playing?'PAUSE':'PLAY'});
  $('timeSlider')?.addEventListener('input',e=>{S.ti=n(e.target.value);timeUI()});
}
...
async function init(){
  ...
  const [cat,times]=await Promise.all([json(`${API}/ocean/catalog`).catch(()=>[]),json(`${API}/ocean/time`).catch(()=>[])]);
  S.catalog=Array.isArray(cat)?cat:(cat.variables||[]);S.times=Array.isArray(times)?times:(times.times||[]);
  ...
  S.renderer.setPixelRatio(Math.min(devicePixelRatio,2));S.renderer.setSize(innerWidth,innerHeight);S.renderer.outputColorSpace=THREE.SRGBColorSpace;
  S.scene.add(new THREE.HemisphereLight(0xffffff,0x6b7b73,1.5));
  const sun=new THREE.DirectionalLight(0xffffff,1.6);sun.position.set(300,900,500);S.scene.add(sun);
  ...
  renderVars();views();ui();timeUI();
  if($('exaggeration'))$('exaggeration').value=70;if($('exagValue'))$('exagValue').textContent='70×';if($('depthMax'))$('depthMax').textContent=`${n(S.g.terrain.maxDepthKm||0).toFixed(1)} km`;if($('depthValue'))$('depthValue').textContent='Sea level → seabed';
